...
**independiente del CLI de IA** que se esté usando.

- En **Claude Code**, estas mismas operaciones se invocan como slash
  commands (`/sync`, `/check`, `/ingest`, `/snapshot`) — la fuente vive en
  `.claude/commands/*.md`.
- En **Codex CLI, Cursor, Cline, Continue u otro CLI sin slash commands**,
  leer este archivo cuando el usuario pida "sincronizar el vault",
  "auditar coherencia", "ingerir un documento" o "tomar un snapshot del
  repo". Las instrucciones canónicas son las de acá.

Las reglas globales del sistema están en `AGENTS.md` (pre-flight,
jerarquía H1–H5, estados de notas, locked guard). Este archivo solo agrega
el **cómo operar** sin asumir un CLI específico.

---

## Engine determinista (siempre disponible, sin LLM)

Antes de cualquier skill, se puede invocar el engine manualmente:
...
El `post-commit` hook ya corre `report` automáticamente; solo hace falta
invocarlo a mano si el hook está deshabilitado o se está operando offline.

Exit codes: `0` éxito, `1` args/archivo inválido, `2` excede `max_files`,
`3` schema inválido, `4` consistency failures, `5` apply tuvo operaciones
fallidas.
...
**Cuándo**: después de cada commit que toque dominio o decisiones técnicas.
Si fue refactor puro de implementación (H5), `sync` puede proponer 0
cambios y eso está bien.

**Inputs**:
- `.vault-sync/change_report.json` — qué cambió (el hook lo generó).
- `.vault-sync/facts.json` — qué hay en el repo (depende del extractor
  activo).
...
1. **Validar el reporte**:
   ...
   ```
   Si exit != 0, abortar y reportar al usuario.
...
3. **Leer `facts.json`** para entender el estado semántico actual. Los
   campos varían según `facts.extractor`:
...
4. **Si el extractor es `graphify`**, aprovechar el `call_graph` antes de
   proponer cambios. `call_graph[archivo_cambiado]` indica qué funciones
...
5. **Para cada nota en `potentially_affected`** decidir qué cambia:
   ...
   - Si está `locked`, ignorar la propuesta y reportar al usuario.
...
7. **Si los cambios atraviesan jerarquía** (H3 o superior), validar
   coherencia con H1–H2 antes de tocar nada.

8. **Generar `.vault-sync/proposed-changes.json`** con operaciones
   concretas. Las acciones soportadas son:
   ```json
   {
     "operations": [
...
9. **Resumir para el usuario**: qué se actualiza, qué se crea, qué se
   deprecia, qué contradicciones se detectaron (sin resolverlas, solo
   señalarlas).
...
- Si `consistency.structural_checks.failed` no está vacío, mencionarlo
  antes de proponer cambios.
- Idempotencia: si la nota ya tiene el valor que se quiere setear, no
  proponer la operación.
- **No leer el código fuente** salvo necesidad puntual — los facts ya
  tienen lo extraíble deterministicamente.

---

## check — auditoría de coherencia transversal

**Cuándo**: cuando hay dudas de que el código siga reflejando los
requisitos del vault.
...
3. **Para cada par capa-superior → capa-inferior, verificar coherencia
   semántica**:
...
4. **Si el extractor activo es `graphify`** (revisar `facts.json::extractor`),
   ...
   - **Frescura**: comparar mtime de `graphify-out/graph.json` con el
     ...
     desactualizado, advertir al usuario:
     > graph.json desactualizado — ejecutar `graphify extract . --no-cluster`
       antes del próximo sync.
   - **Cobertura semántica**: leer `facts.call_graph` y contrastar contra
...
**Cuándo**: el usuario colocó un PDF / .txt / imagen en `vault/raw/` y se
quiere extraer su contenido semántico al grafo.
...
3. **Leer el `INDEX.md` y las notas relevantes** ya existentes para no
...
- Mantener los enlaces wiki (`[[nota]]`) consistentes con las notas que
  existen en el vault.
...
estructuradas con `status`, `code_path` y wikilinks. Graphify produce
nodos — útiles para queries, no para el contrato semántico que el sistema
mantiene.
...
**Cuándo**: solo en arranque de proyecto, refactor masivo, o sospecha de
drift severo. **Caro en tokens** — usar con criterio.

**Antes de leer cualquier archivo de código**:

Verificar si `graphify-out/graph.json` existe.
- Si existe, usarlo como mapa estructural. Leer sus primeras ~200 líneas
  con la herramienta nativa de lectura. Empezar por el grafo evita re-leer
  archivos raw que Graphify ya parseó deterministicamente.
- Si no existe, proceder con lectura directa.

**Pasos**:

1. **Leer `vault_sync.config.json`** para conocer `vault_path`, extractor
   activo, `hierarchy_mapping`.
2. **Generar el reporte y los facts**:
   ```
   python scripts/vault_sync.py report
   ```
3. **Leer `.vault-sync/facts.json`** primero — tiene la extracción
   semántica del repo. Qué exactamente está adentro depende del
   extractor:
...
4. **Leer `INDEX.md` y las notas existentes** (`intent/`, `domain/`,
   `decisions/`). No leer el vault completo — solo lo que ya hay.
5. **Mapear código → vault**:
   - Para cada entidad/módulo H4 detectado en `facts.json`, identificar si
...
8. **Resumir y pedir aprobación**. Si se aprueba:
   ```
   python scripts/vault_sync.py apply .vault-sync/proposed-changes.json
   ```

**Reglas estrictas**:
- Nunca modificar notas con `status: locked` (típicamente `SYSTEM.md` e
  `INDEX.md`).
- Nunca borrar una nota; usar `deprecate` si está obsoleta.
- Si una propuesta contradice una nota `locked`, detenerse y reportar la
  contradicción.
- **No leer el vault completo de golpe** — usar el report y las notas
  relevantes.
- Si el extractor es `generic` y aparecen `proposed_files` en facts.json,
  el primer ítem del resumen debería ser:
